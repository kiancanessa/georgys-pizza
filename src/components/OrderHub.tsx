...
type Tamano = 'Chica (8")' | 'Grande (12")' | 'Familiar (16")';
const TAMANOS: Tamano[] = ['Chica (8")', 'Grande (12")', 'Familiar (16")'];
const OCASIONES = [
  "Cumpleaños 🎂", "Aniversario 💑", "Reunión familiar 👨‍👩‍👧",
  "Cena de negocios 💼", "Graduación 🎓", "Boda 💍",
  "XV Años 👑", "Evento corporativo 🏢", "Otra ocasión",
];
...
function FormReserva({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const msg = [
      ...
      eventoCustom ? `📋 *Detalles del evento:* ${eventoCustom}` : null,
  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex flex-wrap gap-2">
            {OCASIONES.map(o => (
              <motion.button key={o} type="button"
                ...
                className={`font-sans text-[10px] tracking-[.08em] px-3 py-1.5 rounded-sm border transition-all duration-200 ${ocasion === o
        ...
        {/* Campo libre para descripción del evento */}
        {ocasion && (
          <motion.div
          ...
          >
            <label className="block font-sans text-[9px] tracking-[.22em] uppercase text-muted mt-2 mb-1">
              Cuéntanos más sobre tu evento
            </label>
            <textarea rows={2} value={eventoCustom}
              onChange={e => setEventoCustom(e.target.value)}
              placeholder="Número de invitados, decoración especial, menú preferido, horario..."
              className="w-full border-b-2 border-gold/25 bg-transparent py-2 text-ink font-sans text-sm placeholder:text-muted/40 outline-none resize-none" />
          </motion.div>
        )}
...
function FormPedido({ onClose }: { onClose: () => void }) {
  ...
  const [base, setBase] = useState("Salsa de tomate");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const msg = [
      ...
      custom
        ...
        : `🍽️ *Platillo:* ${platillo}`,
      (!custom || sel?.categoria === "Pizza") ? `📏 *Tamaño:* ${tamano}` : null,
      `🛵 *Tipo:* ${tipo}`,
  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Toggle normal / personalizada */}
        <div className="flex gap-2 mb-1">
          <button type="button" onClick={() => setCustom(false)}
            className={`flex-1 font-sans text-[9px] tracking-[.15em] uppercase py-2 rounded-sm border transition-all ${!custom ? "bg-tomato border-tomato text-cream" : "border-gold/20 text-parchment/50"}`}>
            Del menú
          </button>
          <button type="button" onClick={() => setCustom(true)}
            className={`flex-1 font-sans text-[9px] tracking-[.15em] uppercase py-2 rounded-sm border transition-all ${custom ? "bg-gold border-gold text-ink" : "border-gold/20 text-parchment/50"}`}>
            🎨 Personalizada
          </button>
        </div>
        ...
        {!custom ? (
          <div>
            ...
            <select required value={platillo} onChange={e => setPlatillo(e.target.value)} className={inp + " cursor-pointer"}>
              ...
              <optgroup label="🫕 Lasagnas" style={{ background: "#1A0900" }}>
                {menu.filter(p => p.categoria === "Lasagna").map(p => (
                  <option key={p.nombre} value={p.nombre} style={{ background: "#1A0900" }}>
                    {p.nombre} — ${p.precio} MXN
                  </option>
                ))}
          ...
          <div className="space-y-3 border border-gold/20 rounded-sm p-3">
            <p className="font-sans text-[9px] tracking-[.2em] uppercase text-gold">🎨 Pizza Personalizada</p>
            <div>
              <label className="block font-sans text-[9px] tracking-[.15em] uppercase text-parchment/35 mb-1">Base *</label>
              <select value={base} onChange={e => setBase(e.target.value)} className={inp + " cursor-pointer"}>
                {["Salsa de tomate", "Crema", "BBQ", "Sin salsa"].map(b => (
                  <option key={b} value={b} style={{ background: "#1A0900" }}>{b}</option>
                ))}
              </select>
            </div>
            <div>
              <label className="block font-sans text-[9px] tracking-[.15em] uppercase text-parchment/35 mb-1">
                Ingredientes *
              </label>
              <p className="font-sans text-[9px] text-parchment/30 mb-1 italic">
                Disponibles: Peperoni · Jamón · Salami · Champiñones · Chile morrón · Cebolla · Chorizo · Tocino · Salchicha italiana
              </p>
              <textarea rows={2} value={ingredientes} onChange={e => setIngredientes(e.target.value)}
                placeholder="Ej: peperoni, champiñones, cebolla morada..."
                className={inp + " resize-none"} required={custom} />
            </div>
          </div>
        )}
    ...
    <section id="ordenar" className="relative overflow-hidden">
      ...
      <div className="grid grid-cols-2 min-h-[320px] md:min-h-[360px]">
        ...
        <motion.div
          ...
          className={`relative flex flex-col items-center justify-center gap-4 cursor-pointer overflow-hidden transition-all duration-500 section-linen ${zone === "reserva" ? "min-h-[200px] md:min-h-[260px]" : "min-h-[320px] md:min-h-[360px]"
            }`}
        ...
        <motion.div
          ...
          className={`relative flex flex-col items-center justify-center gap-4 cursor-pointer overflow-hidden transition-all duration-500 section-wood ${zone === "pedido" ? "min-h-[200px] md:min-h-[260px]" : "min-h-[320px] md:min-h-[360px]"
            }`}
